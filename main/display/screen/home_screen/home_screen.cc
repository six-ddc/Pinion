...
// Claw6 single-app firmware: every other screen (chat/wifi/digital_people/
// call/music/calendar/openclaw/gps/camera/spirit_level/magnet/vibrate/
// bluetooth/calculator/weather/sd/pin/2048/backlight/info/theme/stock/ebook)
// was a home-menu UI app and is deleted; pi is the only one left. The
// underlying hardware capability each of those exposed (Backlight,
// bluetooth radio, WiFi/4G stack, battery gauge, etc.) lives in boards/
// common and is untouched -- only their screen/ UI layer is gone.
...
// Claw6 single-app firmware: only pi's lifecycle callback remains. All the
// other per-screen callbacks (game_2048/calculator/call/calendar/music/
// weather/gps/camera/vibrate/bluetooth/wifi(network_screen)/chat/
// digital_people/sd_card/pin_test/level/magnet/openclaw/theme/backlight/
// info/stock/ebook) were removed along with their screens.
...
// 主屏 app 表：Claw6 单应用固件只剩 pi 一项（其余 22 项 UI app 已删除，
// 硬件层不受影响）。icon_suffix 复用现有 "chat" 图标资源，无需新图。
