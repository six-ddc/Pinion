...
std::string BuildXmlSystemPrompt() {
    return std::string(kPromptIntro) +
        ...
        "every block takes fill?/id?/hidden?):\n"
        "- <grid>leaves</grid> -- a flow wrapped by size. Use for headers/control rows "
        ...
        "-- close | set:path=value | toggle:id / show:id / hide:id (a hidden leaf or block) | "
        "invoke:cmd. "
