...
//
//   Awake --Chat 视图无操作 120s（固定）--> hooks.chat_timeout()（Go(Idle)，会话保留）
//   Awake（Idle 视图）--无操作达 NVS "ui"/"sleep_s" 秒（0=永不）--> Dim（亮度 20%，不持久化）
//   Dim --再 60s--> Off（亮度 0；时钟/动画深度降耗由 hooks.on_off 负责）
//   Dim/Off --任意触摸 或 PWR_KEY--> Awake（恢复进入 Dim 前缓存的用户亮度）
//
// 三道闸门（hooks.is_gated：生成中 / TTS 播报中 / 快捷面板·设置栈·新对话
// sheet 打开 / 聆听中）期间不进 Dim/Off，且无操作计时持续重置。
//
// 唤醒语义：
...
//
// 平台无关：仅依赖 lvgl + settings.h + mhal::backlight（sim 均有 shim）。
// ---------------------------------------------------------------------------
namespace pi_sleep {
...
struct Hooks {
    ...
    bool (*is_chat)() = nullptr;         // 当前是否 Chat 视图（120s 回待机）
    bool (*is_idle_view)() = nullptr;    // 当前是否 Idle 视图（只有它进 Dim/Off）
    void (*chat_timeout)() = nullptr;    // Chat 无操作 120s（调用方 Go(Idle)）
    void (*on_dim)(bool dim) = nullptr;  // 进/出 Dim（防烧屏移位、呼吸点暂停）
    void (*on_off)(bool off) = nullptr;  // 进/出 Off（停时钟等深度降耗）
};
