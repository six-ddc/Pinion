// stock_api.h
// 腾讯财经零认证行情接口的同步抓取。（移植自 Claw4）
// HTTP 走 stock_http 双端抽象（真机 esp_http_client / sim libcurl）；JSON 用 cJSON
// （5 日分时因响应 ~200KB 改为字符串扫描，见 .cc）。所有函数同步阻塞，只能在
// 后台线程（agent worker / 抓取 worker）调用，绝不可在 LVGL 线程调用。
