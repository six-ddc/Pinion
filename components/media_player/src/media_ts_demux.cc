...
void TsDemux::Feed(const uint8_t* data, size_t len, std::vector<uint8_t>& adts_out) {
    ...
    size_t off = 0;
    bool resynced = false;  // 本次 Feed 是否跳过过垃圾字节（重同步过）
    while (carry_.size() - off >= kPkt) {
        ...
        const bool have_next = carry_.size() - off >= kPkt + 1;
        // 免步长（尾巴不够验下一包）只放行未重同步过的干净尾包；重同步后的不可验证尾巴
        // 留在 carry_ 等下次 Feed 补字节再验，别把伪包直通。
        const bool stride_ok = have_next ? carry_[off + kPkt] == kSync : !resynced;
        if (!head_ok || !stride_ok) {
            if (!have_next && head_ok && resynced) break;  // 重同步后的尾巴：留待下次 Feed
            // 失步：从下一字节起扫描 0x47，同样带跨包步长校验（抗 payload 中的伪同步字节）。
            size_t scan = off + 1;
            while (scan < carry_.size() && carry_[scan] != kSync) scan++;
            resynced = true;  // 已跳过垃圾，之后的免步长尾包不再放行
...
void TsDemux::ParsePacket(const uint8_t* pkt, std::vector<uint8_t>& adts_out) {
    ...
    if (pusi) {
        ...
        // 头损坏/越界：丢弃至下一个 PUSI，否则后续 continuation 包的 184B 垃圾会直通进 ADTS。
        if (kPkt - p < 9 || pkt[p] != 0x00 || pkt[p + 1] != 0x00 || pkt[p + 2] != 0x01) {
            drop_until_pusi_ = true;
            return;
        }
...
void TsDemux::ParsePmt(const uint8_t* payload, size_t len) {
    ...
    p += 12 + prog_info_len;
    // 遍历全部 ES 条目找 0x0F（ADTS AAC）——命中即锁定该 PID 并停（优先级最高）。
    // 扫完全表无 0x0F 时置不支持标志，并记录见过的首个 stream_type（一个 ES 都没有也置不支持）。
    int first_stream_type = 0;
    bool saw_es = false;
    while (p + 5 <= end - 4) {
        ...
        if (!saw_es) {
            saw_es = true;
            first_stream_type = stream_type;
        }
        p += 5 + es_info_len;
    }
    // 无 0x0F：记录首个见过的类型；一个 ES 都没有则记 -1（用非零以触发 found_unsupported）。
    unsupported_stream_type_ = saw_es ? first_stream_type : -1;
}
