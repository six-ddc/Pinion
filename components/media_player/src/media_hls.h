...
//   - 直播滑窗：MEDIA-SEQUENCE 追踪、耗尽后按 TARGETDURATION 节奏刷新、落后窗口跳回
//     直播沿；#EXT-X-ENDLIST（点播）按曲末 EOF 处理；
...
struct HlsPlaylist {
    ...
    bool endlist = false;                   // #EXT-X-ENDLIST（点播/已停播）
...
// RFC3986 的实用子集：ref 带 scheme 则透传；以 '/' 开头则接 base 的 scheme+host；
// 否则接 base 的目录（base 的 query 先剥掉再找最后一个 '/'）。ref 的 query 原样保留。
