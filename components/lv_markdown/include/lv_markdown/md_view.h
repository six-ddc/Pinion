...
class MdView {
  public:
    ...
    // Swap the theme and repaint the whole view: colors are baked into
    // widgets/recolor markup at render time, so this deletes every block
    // widget and re-renders it -- finalized blocks from their retained Block
    // data, the open tail from the parser. Works mid-stream and after
    // Finish(). Caller re-syncs layout/cursor on its next tick as usual.
    void Retheme(const MdTheme& theme);
  ...
  private:
    ...
    MdTheme theme_;
    std::vector<Block> finalized_blocks_;  // retained for Retheme re-rendering
    int32_t width_ = 0;
