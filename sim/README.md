...
| 设备 | 模拟器 |
|---|---|
| PWR_KEY 单击 | **F1**（待机→聆听 / 聆听→取消 / chat→聆听或 STOP） |
...
| NVS（zen_mode / tts_on） | `pi_sim_settings.ini`（`PI_SIM_SETTINGS` 可改路径） |
...
└── shim/
    ...
    │                  # metalio_hal/audio_pipeline pi_esp32 sim_hooks
    └── src/           # pthread 版 FreeRTOS、文件版 Settings、F1=PWR_KEY、
                       # 打字=ASR、TTS 打印/say、pi_esp32_*→pi_posix_*
```
...
错误横幅/重试、ZEN、上下文表、token 用量）全部走真实代码路径。
